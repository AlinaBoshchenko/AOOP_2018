...
public class RemoveBoxAction implements ActionListener {
    private BoxManagerView boxManagerView;
    RemoveBoxAction(BoxManagerView boxManagerView) {
    ...
    public void actionPerformed(ActionEvent actionEvent) {
        ...
        if(serverAddressPort.length ==2) {
            try {
                boxManagerView.getManager().removeBox(new Address(serverAddressPort[0], Integer.parseInt(serverAddressPort[1])));
            ...
            try {
                ...
                if (n < boxManagerView.getManager().getBoxesAddresses().size()) {
                    boxManagerView.getManager().removeBox(n);
                }
            } catch (NumberFormatException e) {
